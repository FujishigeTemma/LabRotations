...
from recorders import ActionPotentialRecorder, VoltageRecorder
...
def simulate_dentate_gyrus(output: str):
    ...
    temporal_patterns = [(inhomogeneous_poisson_process(t_start=0, t_stop=0.5, sampling_interval=0.0001, rate_profile_frequency=10, rate_profile_amplitude=100)* 1000).round(1) for _ in range(n_connection_patterns)]
    ...
    fig = vr.plot()
    fig.savefig(os.path.join(output, "dentate_gyrus_voltage.png"))

    fig = apr.plot()
    fig.savefig(os.path.join(output, "dentate_gyrus_action_potentials.png"))